...
// ?? Seed Data ?????????????????????????????????????????????????
// Migrations + roles + SuperAdmin always run.
// Demo/sample data only runs in Development — never in Production/Railway.
await SeedService.SeedAsync(app.Services, app.Environment.IsDevelopment());
